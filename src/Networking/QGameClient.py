...
class QGameClient(EventBasedClient):
    def __init__(self, delegate, gameID, fastForward=False):
        EventBasedClient.__init__(self, PTG.Client, settingsManager.settings['hostname'], 42142, delegate, fastForward=fastForward)
    ...
    def syncDecks(self):
        directories = ['decks','ygoDecks'] # decks = Magic, ygoDecks = Yu-Gi-Oh
        for directory in directories:
            self.syncDeckDirectory(directory)
        
    def syncDeckDirectory(self,directory):
        from PyQt4.QtCore import pyqtRemoveInputHook
        import pdb
        # pyqtRemoveInputHook()
        # pdb.set_trace()
        try:
            os.makedirs(os.path.join('src','userdata',directory))
        except Exception:
            pass
        localDecks = os.listdir(os.path.join('src',"userdata",directory))
        remoteDecks = self.client.listDecks(directory)
        for ld in localDecks:
            if ld in ("Draft"):
                continue
            f = open(os.path.join('src','userdata', directory, ld), 'r')
            line = f.readline()
            if not line.startswith('timeStamp:'):
                f.seek(0)
                self.client.saveDeck(ld, f.read(), directory)
            f.close()
        for rd in remoteDecks:
            deckContents = self.client.getDeck(directory, rd)
            try:
                f = open(os.path.join('src','userdata',directory, rd), 'r')
                ...
                if serverTime > localTime:
                    f = open(os.path.join('src','userdata', directory, rd), 'w')
