...
class ScrollBar(pygame.sprite.DirtySprite):
    ...
    def setWorldWidth(self, worldDim, returnToZero = True):
        """self.knob = pygame.Rect(self.rect)
        self.ratio = 1.0 * self.image.get_size()[self.axis] / worldDim
        knoblist = list(self.rect.size)
        knoblist[self.axis] = (self.knob.size[self.axis] * self.ratio)
        self.knob.size = knoblist
        self.dirty = True
        self.prettySize = [
            self.knob.width - (self.pad * 2), self.knob.height - (self.pad * 2)]
        self.prettySize[self.oppAxis] = self.thick - (2 * self.pad)
        self.leftTop = [0,0]
        
        """
        #print "Start",self.knob
        knobRectOld = self.knob
        ...
        self.diff[self.axis] = self.initTopleft[self.axis]
        if not returnToZero:
            self.knob.left = max(knobRectOld.left - (worldDim - self.oldDim),0) #### IT NO WORKY!!!!!!!!!!!!! STILL!!!
            desiredLeftTop = self.leftTop[self.axis] + (worldDim - self.oldDim)
            desiredLeftTop = min(desiredLeftTop, worldDim)
            desiredLeftTop = max(desiredLeftTop, 0)
            desiredKnobMove = desiredLeftTop * self.ratio
            self.scroll(desiredKnobMove) # check math
        # Maybe work backwards - knowing what self.leftTop (from get_scrolled) should be, calculate everything else as seen in self.scroll
        #print "End",self.knob
        self.oldDim = worldDim
